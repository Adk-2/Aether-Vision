"""Measured benchmark for identity stability under seeded label noise."""
...
def evaluate(
    ...
    """Run the real identity resolver on seeded noisy multi-track detections."""
    ...
    for frame_index, frame in enumerate(scripted_frames):
        ...
        identities = {
            identity.track_id: identity for identity in resolver.update(tracks)
        }
        for observation in frame.observations:
            total += 1
            if identities[observation.track_id].current_label == observation.expected_label:
                correct += 1
    ...
    return Benchmark(
        ...
        description=(
            "Real IdentityResolver on a fixed-seed >=200-frame, two-track noisy "
            "stream including a low-confidence noise burst; adaptation is measured "
            "on a 15-frame cup then 15-frame bottle relabel."
        ),
        expected_result=(
            "Identity labels should match ground truth during noisy frames and "
            "switch after a real relabel."
        ),
        actual_result=(
...
def run() -> Benchmark:
    """Return the measured identity stability benchmark."""
    return evaluate()
...
def _frames_until_switch(frames: list[MultiTrackFrame]) -> int | None:
    ...
    for frame_index, frame in enumerate(frames):
        ...
        if (
            ...
            and identities[0].current_label == observation.expected_label
        ):
